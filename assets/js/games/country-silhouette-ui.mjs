...
function renderDailyResult(){
  ...
  const share=element('button','btn',text.share),status=element('span','share-status'),training=element('button','btn secondary',text.training);share.type=training.type='button';share.addEventListener('click',async()=>{const rows=session.rounds.map(round=>round.status==='given-up'?'⬛':round.score===100?'🟩':'🟨').join(' '),payload=`MetaphAI · ${text.title}\n${date}\n${rows}\n${summary.score}/500\nhttps://metaphai.com${GAME_PAGES.countrySilhouette[language]}`,result=await shareResult(payload);status.textContent=result==='copied'?text.copied:''});training.addEventListener('click',renderTrainingStart);box.append(share,status,training);appendDailyPushControls(box,language,'countrySilhouette');currentArea().replaceChildren(box)
}
