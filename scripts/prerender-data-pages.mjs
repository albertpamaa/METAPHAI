...
const COMPARE_CSS_VERSION = '20260914-5';
const MAP_ASSET_VERSION = '20260914-7';
const CHANGE_ASSET_VERSION = '20260914-8';
...
function mapPage(language,registry,countries,dataBySlug){
  ...
  return pageChrome({language,pageKey:'map',routes,title:text.title,description:text.intro,body:enhancedBody,schemaType:'WebPage',interactiveScript:'map'}).replace(`data.css?v=${ASSET_VERSION}`,`data.css?v=${MAP_ASSET_VERSION}`);
}
...
function changesPage(language,registry,countries,dataBySlug){
  ...
  return pageChrome({language,pageKey:'changes',routes,title:text.seoTitle,description:text.description,body,schemaType:'WebPage',interactiveScript:'changes'}).replace(`data.css?v=${ASSET_VERSION}`,`data.css?v=${CHANGE_ASSET_VERSION}`);
}
...
function comparePage(language,countries,registry,dataBySlug){
  ...
  const routes=Object.fromEntries(Object.keys(DATA_LANGUAGES).map(code=>[code,DATA_PAGES.compare[code]]));return pageChrome({language,pageKey:'compare',routes,title:text.compareTitle,description:text.compareDesc,body:body.replace('<section class="data-hero">','<section class="data-hero compare-hero">')}).replace(`data.css?v=${ASSET_VERSION}`,`data.css?v=${COMPARE_CSS_VERSION}`);
}
