...
const DATASET_DESCRIPTION={
  es:(name,definition,range)=>`${name}. ${definition} Serie histórica por países y territorios para ${range}, basada en World Development Indicators del World Bank. MetaphAI procesa y presenta los datos para consulta, comparación, rankings y descarga.`,
  en:(name,definition,range)=>`${name}. ${definition} Historical series by country and territory for ${range}, based on the World Bank's World Development Indicators. MetaphAI processes and presents the data for consultation, comparison, rankings and download.`,
  fr:(name,definition,range)=>`${name}. ${definition} Série historique par pays et territoire pour ${range}, fondée sur les World Development Indicators de la Banque mondiale. MetaphAI traite et présente les données pour la consultation, la comparaison, les classements et le téléchargement.`,
  de:(name,definition,range)=>`${name}. ${definition} Historische Reihe nach Ländern und Gebieten für ${range}, basierend auf den World Development Indicators der Weltbank. MetaphAI verarbeitet und präsentiert die Daten für Abfragen, Vergleiche, Rankings und Downloads.`,
  it:(name,definition,range)=>`${name}. ${definition} Serie storica per paesi e territori per il periodo ${range}, basata sui World Development Indicators della Banca Mondiale. MetaphAI elabora e presenta i dati per consultazione, confronto, classifiche e download.`,
  pt:(name,definition,range)=>`${name}. ${definition} Série histórica por países e territórios para ${range}, baseada nos World Development Indicators do Banco Mundial. A MetaphAI processa e apresenta os dados para consulta, comparação, classificações e descarga.`,
  ru:(name,definition,range)=>`${name}. ${definition} Исторический ряд по странам и территориям за ${range} годы на основе World Development Indicators Всемирного банка. MetaphAI обрабатывает и представляет данные для просмотра, сравнения, рейтингов и скачивания.`,
  'zh-CN':(name,definition,range)=>`${name}。${definition} 按国家和地区提供 ${range} 年历史序列，数据来自世界银行 World Development Indicators。MetaphAI 对数据进行处理和展示，以便查询、比较、排名和下载。`,
  hi:(name,definition,range)=>`${name}। ${definition} ${range} के लिए देशों और क्षेत्रों की ऐतिहासिक शृंखला, विश्व बैंक के World Development Indicators पर आधारित। MetaphAI डेटा को देखने, तुलना, रैंकिंग और डाउनलोड के लिए संसाधित और प्रस्तुत करता है।`,
  ja:(name,definition,range)=>`${name}。${definition} 世界銀行の World Development Indicators に基づく、${range} 年の国・地域別時系列です。MetaphAI は閲覧、比較、ランキング、ダウンロードのためにデータを処理して表示します。`,
  ko:(name,definition,range)=>`${name}. ${definition} 세계은행 World Development Indicators를 기반으로 한 ${range}년 국가·지역별 시계열입니다. MetaphAI는 조회, 비교, 순위 및 다운로드를 위해 데이터를 처리하고 제공합니다.`,
  ca:(name,definition,range)=>`${name}. ${definition} Sèrie històrica per països i territoris per a ${range}, basada en els World Development Indicators del Banc Mundial. MetaphAI processa i presenta les dades per a consulta, comparació, rànquings i descàrrega.`,
  ar:(name,definition,range)=>`${name}. ${definition} سلسلة تاريخية حسب البلدان والأقاليم للفترة ${range}، استنادًا إلى مؤشرات التنمية العالمية للبنك الدولي. تعالج MetaphAI البيانات وتعرضها للاستعلام والمقارنة والتصنيفات والتنزيل.`,
  id:(name,definition,range)=>`${name}. ${definition} Seri historis menurut negara dan wilayah untuk ${range}, berdasarkan World Development Indicators dari World Bank. MetaphAI memproses dan menyajikan data untuk penelusuran, perbandingan, peringkat, dan unduhan.`,
  bn:(name,definition,range)=>`${name}। ${definition} বিশ্বব্যাংকের World Development Indicators-এর ভিত্তিতে ${range} সময়ের দেশ ও অঞ্চলভিত্তিক ঐতিহাসিক সিরিজ। MetaphAI তথ্য অনুসন্ধান, তুলনা, র‍্যাঙ্কিং ও ডাউনলোডের জন্য প্রক্রিয়া ও উপস্থাপন করে।`
};
...
function indicatorDatasetSchema(language,item,data,route){
  const localized=localizedIndicator(item,language),range=`${data.coverage.min_year}–${data.coverage.max_year}`,describe=DATASET_DESCRIPTION[language]||DATASET_DESCRIPTION.en;
  return{
    '@type':'Dataset',
    name:localized.name,
    description:describe(localized.name,localized.description,range),
    url:`${ORIGIN}${route}`,
    identifier:item.code,
    creator:{'@type':'Organization',name:'World Bank'},
    publisher:{'@type':'Organization',name:'MetaphAI',url:`${ORIGIN}/`},
    license:DATASET_LICENSE,
    temporalCoverage:`${data.coverage.min_year}/${data.coverage.max_year}`,
    variableMeasured:localized.name,
    includedInDataCatalog:{'@type':'DataCatalog',name:'World Development Indicators',url:'https://datacatalog.worldbank.org/search/dataset/0037712/world-development-indicators'},
    isBasedOn:'https://datacatalog.worldbank.org/search/dataset/0037712/world-development-indicators',
    isAccessibleForFree:true,
    inLanguage:language
  };
}

function upsertDatasetSchema(html,schema){
  let replaced=false;
  const standalone={'@context':'https://schema.org',...schema};
  const updated=html.replace(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g,(full,json)=>{
    let value;try{value=JSON.parse(json)}catch{return full}
    if(value?.['@type']==='Dataset'){replaced=true;return `<script type="application/ld+json">${JSON.stringify(standalone).replace(/</g,'\\u003c')}</script>`}
    const graph=value?.['@graph'];if(!Array.isArray(graph))return full;
    const index=graph.findIndex(entry=>entry?.['@type']==='Dataset');if(index<0)return full;
    graph[index]=schema;replaced=true;return `<script type="application/ld+json">${JSON.stringify(value).replace(/</g,'\\u003c')}</script>`;
  });
  if(replaced)return updated;
  return updated.replace('</head>',`<script type="application/ld+json">${JSON.stringify(standalone).replace(/</g,'\\u003c')}</script></head>`);
}
...
function pageChrome({ language, pageKey, routes, title, description, body, schemaType = 'WebPage', primarySchema=null, bodyAttributes='', interactiveScript='views' }) {
  ...
  const schemaPrimary=primarySchema||{'@type':schemaType,name:title,url:`${ORIGIN}${route}`,isBasedOn:'https://datacatalog.worldbank.org/search/dataset/0037712/world-development-indicators'};
  const jsonLd = JSON.stringify({'@context':'https://schema.org','@graph':[schemaPrimary,{'@type':'BreadcrumbList',itemListElement:breadcrumbItems}]}).replace(/</g, '\\u003c');
...
function indicatorPage(language,pageKey,item,data,countries,registry){
  ...
  return pageChrome({language,pageKey,routes,title,description,body,primarySchema:indicatorDatasetSchema(language,item,data,routes[language]),bodyAttributes:`data-indicator="${item.slug}"`,interactiveScript:'explorer'});
}
...
function countryPage(language, country, countries, registry, dataBySlug) {
  ...
  return pageChrome({language,pageKey:`country:${country.id}`,routes,title:`${localizedCountry.name} — ${text.countrySubtitle}`,description:`${localizedCountry.name}: ${text.summary}. ${text.sourceText}`,body,schemaType:'WebPage'});
}
...
export async function prerender() {
  ...
  for (const [pageKey,page] of Object.entries(DATA_PAGES).filter(([,candidate]) => candidate.indicator)) {
    ...
    for (const language of Object.keys(DATA_LANGUAGES)) {
      ...
      html = upsertDatasetSchema(html, indicatorDatasetSchema(language,item,dataBySlug.get(item.slug),page[language]));
