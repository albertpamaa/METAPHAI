...
for(const language of languages){const route=DATA_PAGES.map[language],html=read(join(...route.split('/').filter(Boolean),'index.html')),locale=languageConfig(language).locale,localized=VIEWS.localizedCountries(countries,locale);for(const token of ['class="map-country-search-icon" aria-hidden="true"','<svg viewBox="0 0 24 24"','id="map_country" type="search"','role="combobox"','aria-autocomplete="list"','aria-controls="map_country_options"','id="map_country_options"','role="listbox"','data.css?v=20260914-7','data-map.js?v=20260914-7'])assert.ok(html.includes(token),`${language}: contrato combobox/asset ausente: ${token}`);assert.equal(localized.length,217,`${language}: deben existir 217 entidades`);assert.deepEqual(localized.map(country=>country.name),localized.map(country=>country.name).toSorted((a,b)=>a.localeCompare(b,locale)),`${language}: orden localizado incorrecto`)}
...
assert.match(runtime,/input\.value\.trim\(\)\?CORE\.searchCountries\(state\.localized,input\.value,\[\],12,i18n\.countrySearchAliases\):state\.localized/,'vacío debe mostrar las 217 entidades; con texto conserva el filtro');assert.doesNotMatch(runtime,/state\.localized\.slice\(0,12\)/);
const selectedRules=[...css.matchAll(/\.map-country\.is-selected[^}]*\{([^}]*)\}/g)].map(match=>match[1]).join(' ');assert.doesNotMatch(selectedRules,/\bfill\s*:/,'selected no debe alterar el fill estadístico');assert.match(css,/\.map-country\.is-selected:hover/);assert.match(css,/\.map-country-picker \.map-country-options\{[^}]*width:100%[^}]*max-height:240px/);assert.match(css,/\.country-options\{[^}]*max-height:280px[^}]*overflow:auto/);assert.match(css,/\[dir="rtl"\] \.map-country-picker input/);
