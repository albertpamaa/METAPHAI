...
const routes=[...Object.values(DATA_PAGES).flatMap(page=>languages.map(language=>page[language])),...Object.values(INSTITUTIONAL_PAGES).flatMap(page=>languages.map(language=>page[language]))];
assert.equal(routes.length,480);assert.equal(new Set(routes).size,480);
...
console.log('multilingual-http: 480/480 URLs base HTTP 200 y scripts 15 idiomas válidos');
