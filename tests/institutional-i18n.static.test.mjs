...
const substantial={privacy:{h2:14,p:16,li:10},contact:{h2:2,p:4,li:0},about:{h2:4,p:9,li:0}};
...
for(const [key,page] of identities)for(const lang of languages){
  ...
  if(key==='privacy'){assert.ok((html.match(/mailto:info@metaphai\.com/g)||[]).length>=1,`${route}: contacto de privacidad ausente`);assert.equal(html.includes('adsbygoogle.js'),false);assert.equal(html.includes('googletagmanager.com/gtag/js'),false)}else if(key==='contact'){assert.equal(html.includes('adsbygoogle.js'),false,`${route}: AdSense no debe cargarse en Contacto`);assert.ok(html.includes('googletagmanager.com/gtag/js?id=G-1P9N4QY0JR'))}else{assert.ok(html.includes('adsbygoogle.js?client=ca-pub-7545567251029894'));assert.ok(html.includes('googletagmanager.com/gtag/js?id=G-1P9N4QY0JR'))}
