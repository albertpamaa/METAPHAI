...
const root=resolve(import.meta.dirname,'..'),origin='https://metaphai.com',version='20260912-8';
...
assert.equal(identities.length,3);assert.equal(languages.length,6);
assert.equal(new Set(identities.flatMap(([,page])=>languages.map(lang=>page[lang]))).size,18);
for(const [key,page] of identities)for(const lang of languages){
  ...
  if(key==='privacy'){assert.equal((html.match(/mailto:info@metaphai\.com/g)||[]).length,2);assert.equal(html.includes('adsbygoogle.js'),false);assert.equal(html.includes('googletagmanager.com/gtag/js'),false)}else{assert.ok(html.includes('adsbygoogle.js?client=ca-pub-7545567251029894'));assert.ok(html.includes('googletagmanager.com/gtag/js?id=G-1P9N4QY0JR'))}
  if(key==='contact'){assert.ok(html.includes('href="mailto:info@metaphai.com"'));assert.ok(html.includes("navigator.clipboard.writeText('info@metaphai.com')"));assert.match(html,/data-copy-label="[^"]+" data-copied-label="[^"]+"/)}
...
console.log('institutional-i18n-static: 18 páginas, 3 equivalencias, SEO, selector, footers, email y enlaces OK');
