...
for(const language of languages){
  ...
  for(const [pageKey,routes] of Object.entries(GAME_PAGES)){
    ...
    if(pageKey==='home'){
      ...
      assert.ok(html.includes('data-geo-quiz'));assert.ok(html.includes('data-game-stats'));assert.ok(html.includes(stats.statistics));assert.ok(html.includes(geo.howTitle));assert.ok(html.includes('geo-quiz-ui.mjs?v=20260917-4'));assert.ok(html.includes('Wikidata (CC0)'));
    }
